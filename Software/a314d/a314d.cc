...
static inline void set_dir_write()
{
    if (current_dir == 0)
    {
        ...
        *(gpio + 1) = (*(gpio + 1) & ~GPIO_DIR_1_MASK) | GPIO_DIR_1_DOUT;
        current_dir = 1;
    }
}
...
static void gpio_write_reg(unsigned int reg, unsigned int value)
{
    if (current_dir == 0)
    {
        *(gpio + 7) = (1 << PIN_WR);
        *(gpio + 1) = (*(gpio + 1) & ~GPIO_DIR_1_MASK) | GPIO_DIR_1_DOUT;
        *(gpio + 2) = (*(gpio + 2) & ~GPIO_DIR_2_MASK) | GPIO_DIR_2_DOUT;
        current_dir = 1;
    }
...
static unsigned int gpio_read_reg(unsigned int reg)
{
    ...
    {
        *(gpio + 1) = (*(gpio + 1) & ~GPIO_DIR_1_MASK) | GPIO_DIR_1_DIN;
        *(gpio + 2) = (*(gpio + 2) & ~GPIO_DIR_2_MASK) | GPIO_DIR_2_DIN;
        *(gpio + 10) = (1 << PIN_WR);
        current_dir = 0;
    }
